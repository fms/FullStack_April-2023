...
describe('app', () => {
    ...
    const updatedTask = { title: "updated title", description: "updated description",status: TaskStatus.done, id: "updated" };
    ...
    describe('started with one existing task', () => {
        ...
        describe("update", () => {
            test("finding an existing task successfully", async () => {
                updatedTask.id = taskToDelete.id;
                const { status, body } = await request(app).patch(`/api/tasks/update`).send(updatedTask);
                expect(status).toBe(200);
                expect(body.tasks).toContainEqual(updatedTask);
            });
            test("one of the details are empty", async() =>{
            ...
            test("ID not specified", async() =>{
                updatedTask.id = "";
                const {status, body} = await request(app).patch("/api/tasks/update").send(updatedTask);
                expect(status).toBe(500);
                expect(body.errors).toEqual(["The task ID must be specified"]);
            })
        });
