...
// Mission #2
let numberFromUser = parseInt(prompt(`Hey dude give me a number! ╰(*°▽°*)╯`) || "0");
let total = 0;
for (let loopTimes = 1; loopTimes <= numberFromUser; loopTimes++) {
    total = total + loopTimes;
}
console.log(`Well if we took your number (${numberFromUser}) and start plus from 1 to your number ${numberFromUser}, we got summery of ${total} 😀`);
// Mission #3
let numberChoosenByUser = parseInt(prompt(`Let's choose again a number!`) || "0");
let total1 = 0;
...
console.log(`Well if we took your number (${numberChoosenByUser}) and start plus from 1 to your number ${numberChoosenByUser}, we got summery of ${total1} 😀`);
