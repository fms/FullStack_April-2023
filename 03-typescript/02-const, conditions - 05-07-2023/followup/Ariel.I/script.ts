...
const userWeightNumber: string | null =
  prompt("What is your weight on Kg?") || "0";
const userHeightNumber: string | null =
  prompt("What is your height on Cm?") || "0";

const convertWeight = parseFloat(userWeightNumber);
const convertHeight = parseFloat(userHeightNumber);

const calcBmi: number = convertWeight / (convertHeight ^ 2);

if (calcBmi > 25) console.log(`Overweight ${calcBmi}`);
else if (calcBmi < 18) console.log(`Underweight ${calcBmi}`);
else console.log(`Normal ${calcBmi}`);
