...
function findPrimaryNumbers(upperLimit: number): void {
    outer: for (let thisNumber = 2; thisNumber <= upperLimit; thisNumber++) {

        let primary: boolean = true;        // We assume the number is primary

        if (thisNumber % 2 === 0 && thisNumber !== 2) {          // Skips multiplies of 2
            continue;
        }

        let firstDivisor: number = 0;
...
function readNextLineFromFile(): string {
    return "quit";
}
