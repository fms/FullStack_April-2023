...
window.addEventListener("DOMContentLoaded", game); //General sprite load
...
function game() {
  ...
  var canvas = document.getElementById('canvas'),
      ...
      cW = ctx.canvas.width = window.innerWidth; //Game
  ...
  function fire() {
    ...
    for (var i = 0; i < bullets.length; i++) {
      if (!bullets[i].destroyed) {
        ...
        bullets[i].realY += cH / 2; //Collision
  ...
  function _asteroids() {
    ...
    for (var i = 0; i < asteroids.length; i++) {
      if (!asteroids[i].destroyed) {
        ...
        asteroids[i].realY += asteroids[i].coordsY; //Game over
