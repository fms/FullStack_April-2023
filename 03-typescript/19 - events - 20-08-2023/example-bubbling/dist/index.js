 ...
 */
function reportBubble(event) {
    reportStep(event, "Bubble");
}
function reportCapture(event) {
    reportStep(event, "Capture");
}
function reportStep(event, stepName) {
    let clicked = event.target; // The element originating the event
    let actual = event.currentTarget; // The element processing the event (bubbling)
    let prefix = clicked === actual ? "|" : " ";
    console.log(`${prefix}${event.type}-${stepName}${prefix}: ${clicked.tagName} ${actual.tagName}`);
